...
export default function Product({ product }) {
  ...
  return ( 
    <>
      ...
      >
        { product?.url ? <img className="rounded cursor-pointer" src={product.url} /> : null }

        <div className="pt-2 px-1">

          <div className="font-semibold text-[15px] hover:underline cursor-pointer">{product?.title}</div>
          <div className="font-extrabold">Rs {(product?.price).toFixed(2)}</div>

          <div className="relative flex items-center text-[12px] text-gray-500">
              <div className="">Rs {((product?.price * 1.2)).toFixed(2)}</div>
